...
//$total_hours = number_format(( $total_raw_minutes / 60 ), 4, '.', '');
    ...
    # Main plot titles:
    $plot->SetTitle('Noise data for '.str_replace(".levels", ".mp3, generated by sox / awk", $_GET['levels']));
    $plot->SetXTitle('Elapsed Audio MP3 Time In '.$measure . ' ('.$length.' '.$measure.' of data, displaying amplitudes of '.$amplitude_minimum.' or greater)'.( $measure == 'Hours' ? ' 0.05 hours = 3 minutes' : '' ));
